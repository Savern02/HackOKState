...
client = OpenAI(
  ...
  api_key= ai_key.strip().strip('"').strip("'"),

)
def call_openai(html_data: str):
    print(ai_key.strip().strip('"').strip("'"))
    response = client.chat.completions.create(
    model="google/gemini-2.0-flash-lite-001",
    messages=[

            {"role": "system", "content": "You are an HTML parser. Analyze HTML content and extract links, locations, and descriptions."},
            ...
            "schema": {
                ...
                "properties": {
                    ...
                    "description": {
                        ...
                        "description":"Location, followed by a short description of the listing"
...
if __name__ == "__main__":
    ...
    html_body = parseHTML(getHTML())
    result = call_openai(html_body)
    with open(DEFAULT_URL + ".txt", "w") as file:
        file.write(response)
